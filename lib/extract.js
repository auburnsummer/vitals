 ...
 */
const simpleKeyExtractor = (name, key, def) => {
  return (rdlevel) => {
    return {
...
module.exports = (rdlevel, zip, buffer, selectedExtractors) => {
  // default: get all of the extractors
  if (_.isNil(selectedExtractors)) {
    selectedExtractors = _.keys(extractors)
  }
  // run each extractor, building up an object as we go
  return _.reduce(selectedExtractors, async (prev, curr) => {
